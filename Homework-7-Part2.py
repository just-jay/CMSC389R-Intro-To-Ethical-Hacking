...
for line in f: #go through the lines in the file
	for c in ascii_lowercase: #go thorugh all the lowercase letters
		h = hashlib.sha512(c + (line.rstrip())).hexdigest() #remove all end tags from the line, add the salt, and hash
		if h in hashlist: #if the computed hash is in the hash list
			print "Salt:" + c + " Password:" + line #print the salt and it's password
